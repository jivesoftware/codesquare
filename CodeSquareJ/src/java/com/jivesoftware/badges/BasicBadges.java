...
public final class BasicBadges {
    ...
    public void processBadges() throws JSONException, IOException, Exception {
        ...
        for (int i = 0; i < jArrCommits.length(); i++) {
            ...
            if (i == 0) {
                ...
                if (data == null) {
                    ...
                    user = new UserInfo(c.getEmail(), data, c.getCommitDate()
                            .getLocal());
                }
            }
        ...
        System.out.println(newBadges.toString() + "old: "
                + oldBadges.toString());
        if (oldBadges.contains("16")) {
            System.out.println(newBadges.toString() + "2old: "
                    + oldBadges.toString());
        ...
        HbaseTools.addRow(table, user.getEmail(), user.getLastCommit(),
                user.getNumBugs(), user.getNumCommits(),
                user.getConsecCommits(),
                (newBadgesString + badgeList[1]).trim(),
                newBadges.toArray(new String[newBadges.size()]), unixTime);

        String[][] badgesList = ServletTools.getBadgeInfo();
        ArrayList<Badge> badges = new ArrayList<Badge>();
        for (String s : newBadges) {
            Badge badge = new Badge(badgesList[Integer.parseInt(s) - 1][0],
                    badgesList[Integer.parseInt(s) - 1][1], s + ".png");
            badges.add(badge);
        }
        if (!badges.isEmpty()) {
            String jsonActivity = ServletTools.makeJSONPost(badges,
                    user.getName());
     ...
     * 
     * @param newBadges
     *            list of new badges to add to if obtained
     * @param data
     *            row information from the hbase
     * @param unixTime
     *            push time of the commits
     */
    public void checkBadge16(ArrayList<String> newBadges, Result data,
            String unixTime) {
        ...
        for (int i = 0; (i + 7) < badgeDates.size(); i++) {
            int timeDiff = Integer.parseInt(badgeDates.get(i))
                    - Integer.parseInt(badgeDates.get(i + 7));
    ...
    /***
     * Checks for badges 26, jiver=jive in the commit message, 31 commit message
     * is more than 20 words
     * 
     * @param user
     *            user to add badges too
     * @param message
     *            commit message
     */
    public void checkMessageBadges(UserInfo user, String message) {
        if (message.toLowerCase().contains("jive")
                && !message.toLowerCase().contains("Merge branch")) {
            user.addBadge("26");
        }
    ...
    /***
     * This method checks to see if there are commits twice in a day, or greater
     * than 5days badges 27, 28
     * 
     * @param user
    ...
    public void checkBadges27And28(UserInfo user, JiveDate newDate) {
        ...
        long timeDiff = (newDate.getLocal().getTime().getTime() / 1000)
                - oldCommitTime;
    ...
    public void checkTimeBadges(UserInfo user, JiveDate commitDate) {
        ...
        if (commitDate.getLocalDay() == 2 && commitDate.getLocalHour() <= 5) {
            user.addBadge("18");
        } else if (commitDate.getLocalDay() == 6
                && commitDate.getLocalHour() >= 16) {
            user.addBadge("19");
        } else if (commitDate.getLocalDay() == 1
                || commitDate.getLocalDay() == 7) {
            user.addBadge("29");
        }
    }
}