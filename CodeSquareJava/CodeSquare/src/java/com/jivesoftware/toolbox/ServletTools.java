...
public class ServletTools {
    ...
    /**
     * Sends a response back to the requester in JSON format. The data in the
     * json depends on what the request was.
     * 
     * @param response
     *            the http servlet response to a given http servlet request
     * @param j
     *            The JSONObject to be sent as a String to the requester
     * @throws JSONException
     * @throws IOException
     */
    public static void sendJSONOutput(HttpServletResponse response, JSONObject j)
     ...
     * 
     * @param jCommit
     *            the JSON Object containing commit information
     * @param pushDate
     *            the Git Push date of the commit
     * @return A java Commit object
     * @throws JSONException
     */
    public static Commit convertToCommit(JSONObject jCommit, String pushUnixTime)
            throws JSONException {
        Commit commit = new Commit();
        commit.setPushDate(new JiveDate(pushUnixTime));
        String x = jCommit.getString("isotimestamp").split(" ")[2];
        commit.setCommitDate(new JiveDate(jCommit.getString("unixtimestamp"),
                jCommit.getString("isotimestamp")));
    ...
    /**
     * This method takes the list of badges that a person recently obtained and
     * converts the badge data into a nice json object to be posted to the
     * activity stream
     * 
     * @param badgesList
     *            A string list filled with the data of the recently obtained
     *            badges
     * @param name
     *            The name of the person who just received the badges
     * @return a String in JSON format that will be posted to the activity
     *         stream
     **/
    public static String makeJSONPost(List<Badge> badgesList, String name)
            throws Exception {
        String jsonString;
    ...
    /***
     * Retrieves the badge information from a json file called badge
     * descriptions
     * 
     * @return two dimensional array of the badge number and its description
     * @throws IOException
     * @throws JSONException
     */
    public static String[][] getBadgeInfo() throws IOException, JSONException {
        ...
        String[][] badgeList = new String[j1.length()][j1.getJSONObject(0)
                .length()];

        for (int i = 0; i < j1.length(); i++) {
            badgeList[i][0] = (String) j1.getJSONObject(i).getString("name");
            badgeList[i][1] = (String) j1.getJSONObject(i).getString(
                    "description");
        }
