...
/**
 * Retrieves email information from the Jive App and uses it to query the
 * 'Employee' and 'Badges' HBase table
 * 
...
public class AppServlet extends HttpServlet {
    ...
    protected void doGetOrPost(HttpServletRequest request,
        ...
        if (matchFound && match2Found) {
            ...
            if (!bossEmail.equals("noBoss@nomail.com") && name != null
                    && id != null) {
                HbaseTools.addUserOrUpdateBoss(table, email, bossEmail, name,
                        id);
            }
     ...
     * 
     * @param badges
     *            A String array whole elements are acquired badge ID's
     * @param BadgeTable
     *            The table in the HBase containing Badge information
     * @param newBadges
     *            A String containing integers represented the Badge Id of newly
     *            obtained badges delimited by spaces
     * @return a JSONObject whose keys are badge ID, and values are JSON objects
     *         whose keys are Name, Description, URL, and New
     * @throws JSONException
     * @throws IOException
     */
    private static JSONObject convertOutputToJSON(ArrayList<String> badges,
            String newBadges, boolean earnedOnly) throws JSONException,
            IOException {
        ...
        if (!earnedOnly) {
            for (Integer i = 0; i < badges.size(); i++) {
                ...
                j2.put("Thumbnail", "images/thumbnails/" + badges.get(i)
                        + "TH.png");
            ...
            for (Integer k = new Integer(1); k <= badgesList.length; k++) {
                ...
                if (badges.contains(k.toString())) {
                    ...
                    j2.put("Name", badgeInfo[0]);
                    j2.put("Description", badgeInfo[1]);
                    j2.put("Thumbnail", "images/thumbnails/" + k.toString()
                            + "TH.png");
