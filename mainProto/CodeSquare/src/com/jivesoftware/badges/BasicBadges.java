...
public class BasicBadges {
	...
	/***
	 * This method takes various information from a recent Push, and uses it
	 ...
	 */
	public static void checkUpdateBadges(HTable table, Push c, int numBugs) {
		...
		int[] fieldValues = Hbase.getFields(data, new String[] { "badgesWeek", "numBugs", "numPushes", "consecPushes" });
		...
		int[] consecPushes = checkConsecPushes(date, lastCommit, fieldValues[3]);

		badges.addAll(checkNumericalBadges(fieldValues, consecPushes));
		...
		Hbase.addRow(table, c.getEmail(), date, badges.size() + fieldValues[0],
				fieldValues[1], fieldValues[2], consecPushes[0], newBadges,
	 ...
	 * number of pushes, and committed person
	 * 
	 * @param fieldValues
	 *            array of three things, [0] is number of badges in week, [1] is
	 *            bugs, [2] is number of pushes
	 * @param consecPushes
	 *            [0] is committed person, [1] is 2 pushes in a day, [2] is 2
	 *            pushes in >5days
	...
	public static ArrayList<String> checkNumericalBadges(int[] fieldValues,
		...
		int totNumPushes = fieldValues[2];
		...
		if (totNumPushes > 4999) {
			badges.add("5");
		} else if (totNumPushes > 999) {
			badges.add("4");
		} else if (totNumPushes > 499) {
			badges.add("3");
		} else if (totNumPushes > 49) {
			badges.add("2");
		} else if (totNumPushes > 0) {
			badges.add("1");
		}
	...
	/***
	 * This method checks to see if there are pushes on consecutive days, twice
	 ...
	 * @param lastCommit
	 *            Previous Push date
	 * @param consecPushesOld
	 *            Consecutive pushes before the newest Push
	 * @return An integer array, [0] is consecutive pushes, [1] is pushes in a
	 *         day, [2] is pushes in >5days
	 */
	public static int[] checkConsecPushes(String commitDate,
			String lastCommit, int consecPushesOld) {
		int[] result = new int[2]; // result[0] is commited person field, and
									// result[1] is (value = 1)2 pushes in a
									// day or (value = 2)2 pushes in >5 days: 0
									// is none
		...
		if ((date.getTime() - dateOld.getTime()) < check.getTime()
			...
			result[0] = consecPushesOld + 1;
		} else if (date.getTime() == dateOld.getTime()) {
			result[0] = consecPushesOld;
			result[1] = 1;
		}
		try {
