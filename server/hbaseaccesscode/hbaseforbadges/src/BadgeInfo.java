...
public class BadgeInfo {
	/***
	 * This method creates table called Badges and loads the badge information
	 * from a file
	 * 
	 * @param args
	 */
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < 31; i++) {
				addBadge(table, (i + 1) + "", br.readLine(), "images/"
						+ (i + 1) + ".png", br.readLine());
			}
	...
	public static void addBadge(HTable table, String badgeNumber,
			String badgeName, String iconURL, String description) {

		Put row = new Put(Bytes.toBytes(badgeNumber));

		row.add(Bytes.toBytes("Info"), Bytes.toBytes("name"),
				Bytes.toBytes(badgeName));
		row.add(Bytes.toBytes("Info"), Bytes.toBytes("iconURL"),
				Bytes.toBytes(iconURL));
		row.add(Bytes.toBytes("Info"), Bytes.toBytes("description"),
