        		...
        		osapi.jive.core.users.get({id: friendList[j].id}).execute(function(user) {
        			console.log("2: "+count);
        	 		if (!user.error) {
    					...
    					if (nameLength > 25) {
    					document.getElementById(count).innerHTML+="<img src='"+user.data.avatarURL+"' onclick=\"javascript:makeBadgeTable('"+user.data.email+"', '',false,'"+user.data.firstName+"')\" />&nbsp;<a href=\"javascript:makeBadgeTable('"+user.data.email+"', '',false,'"+user.data.firstName+"')\">"+user.data.firstName+"&nbsp;"+user.data.lastName.charAt(0)+".</a> <br/>";
                		}
    					else {
    					document.getElementById(count).innerHTML+="<img src='"+user.data.avatarURL+"' onclick=\"javascript:makeBadgeTable('"+user.data.email+"', '',false,'"+user.data.firstName+"')\" />&nbsp;<a href=\"javascript:makeBadgeTable('"+user.data.email+"', '',false,'"+user.data.firstName+"')\">"+user.data.firstName+"&nbsp;"+user.data.lastName+"</a> <br/>";
                		}
                		count = count+1;
    				}
    			});
...
function makeBadgeTableFromId(userId) { 
    ...
    request.execute(function(viewer) {
    	if (!viewer.error) {
    		makeBadgeTable(viewer.data.email, "", false, viewer.data.firstName);
    	}
    }); 
}
